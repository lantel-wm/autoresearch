...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] behavioral_trading dailymom_turnstable_liq20",
        feature_expressions=[
            ...
            (
                ...
                "(($turnover_rate / (Mean($turnover_rate, 20) + 1e-12)) / "
                "((Std($turnover_rate, 20) / (Mean($turnover_rate, 20) + 1e-12)) + 1e-12)), 20) * "
                "(Mean($turnover_rate, 20) / (Mean(Abs($close / Ref($close, 1) - 1), 20) + 1e-12)))",
                "dailymom_turnstable_liq20",
            ),
            (
