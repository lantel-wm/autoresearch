...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] lowrisk_liquidity negsemivar_turncalm20_120",
        feature_expressions=[
            ...
            (
                "(0 - Mean(Greater(0 - ($close / Ref($close, 1) - 1), 0) * "
                "Greater(0 - ($close / Ref($close, 1) - 1), 0), 20)) * "
                "(Mean($turnover_rate, 120) / (Mean($turnover_rate, 20) + 1e-12))",
                "negsemivar_turncalm20_120",
            ),
            (
