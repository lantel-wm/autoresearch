...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] behavioral_trading losscont_losscrowd20_120",
        feature_expressions=[
            ...
            (
                "(Mean(Greater(0 - ($close / Ref($close, 1) - 1), 0), 20) / "
                "(Max(Greater(0 - ($close / Ref($close, 1) - 1), 0), 20) + 1e-12)) * "
                "Greater(0 - ($close / Ref($close, 20) - 1), 0) * "
                "(Mean($turnover_rate, 20) / (Mean($turnover_rate, 120) + 1e-12))",
                "losscont_losscrowd20_120",
            ),
            (
