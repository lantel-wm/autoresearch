...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] behavioral_trading losspath_illiqshock20_120",
        feature_expressions=[
            ...
            (
                "Greater(0 - ($close / Ref($close, 20) - 1), 0) * "
                "Greater(Mean($close < Ref($close, 1), 20) - Mean($close > Ref($close, 1), 20), 0) * "
                "((Mean(Abs($close / Ref($close, 1) - 1) / ($turnover_rate + 1e-12), 20)) / "
                "(Mean(Abs($close / Ref($close, 1) - 1) / ($turnover_rate + 1e-12), 120) + 1e-12))",
                "losspath_illiqshock20_120",
            ),
            (
