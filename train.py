...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] behavioral_trading lifecycle_turnstable20_60",
        feature_expressions=[
            ...
            (
                ...
                "(($close > Ref($close, 20)) * "
                "((Mean($turnover_rate, 60) - Mean($turnover_rate, 20)) / "
                "(Std($turnover_rate, 20) + 1e-12)) + "
                "($close < Ref($close, 20)) * "
                "((Mean($turnover_rate, 20) - Mean($turnover_rate, 60)) / "
                "(Std($turnover_rate, 20) + 1e-12)))",
                "lifecycle_turnstable20_60",
            ),
            (
