...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] behavioral_trading mindrop_turnstable_losscrowd20_120",
        feature_expressions=[
            ...
            (
                "((0 - Min($close / Ref($close, 1) - 1, 20))) * "
                "((Mean($turnover_rate, 20) / (Mean($turnover_rate, 120) + 1e-12)) / "
                "((Std($turnover_rate, 20) / (Mean($turnover_rate, 20) + 1e-12)) + 1e-12)) * "
                "Greater(0 - ($close / Ref($close, 20) - 1), 0)",
                "mindrop_turnstable_losscrowd20_120",
            ),
            (
