...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] behavioral_trading earlystage_turnmom55",
        feature_expressions=[
            ...
            (
                "($close / Ref($close, 55) - 1) * "
                "(($close > Ref($close, 55)) / "
                "((Mean($turnover_rate, 20) / (Mean($turnover_rate, 55) + 1e-12)) + 1e-12) + "
                "($close < Ref($close, 55)) * "
                "(Mean($turnover_rate, 20) / (Mean($turnover_rate, 55) + 1e-12)))",
                "earlystage_turnmom55",
            ),
            (
                "($close / Ref($close, 5) - 1) * "
