...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] behavioral_trading salience_turn20",
        feature_expressions=[
            ...
            (
                "Min((($close / Ref($close, 1) - 1) * "
                "($turnover_rate / (Mean($turnover_rate, 20) + 1e-12))), 20)",
                "daily_loser_salience20",
            ),
            (
                "Max((($close / Ref($close, 1) - 1) * "
                "($turnover_rate / (Mean($turnover_rate, 20) + 1e-12))), 20)",
                "daily_winner_salience20",
            ),
            (
                "Max(Abs($close / Ref($close, 1) - 1) * "
                "($turnover_rate / (Mean($turnover_rate, 20) + 1e-12)), 20)",
                "daily_abs_salience20",
            ),
            (
                "($close / Ref($close, 10) - 1) * "
