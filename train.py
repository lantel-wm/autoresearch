...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] behavioral_trading downasym_turncrowd20_120",
        feature_expressions=[
            ...
            (
                ...
                "((Std($turnover_rate, 20) / (Mean($turnover_rate, 20) + 1e-12)) + 1e-12)), 20)",
            ...
            (
                "((0 - Min($close / Ref($close, 1) - 1, 20))) * "
                "(Mean($turnover_rate, 20) / (Mean($turnover_rate, 120) + 1e-12)) * "
                "Greater(0 - ($close / Ref($close, 20) - 1), 0)",
                "mindrop_losscrowd20_120",
            ),
            (
                "((Mean((0 - Less($close / Ref($close, 1) - 1, 0)) * "
                "(0 - Less($close / Ref($close, 1) - 1, 0)), 20) - "
                "Mean(Greater($close / Ref($close, 1) - 1, 0) * "
                "Greater($close / Ref($close, 1) - 1, 0), 20)) / "
                "(Mean((0 - Less($close / Ref($close, 1) - 1, 0)) * "
                "(0 - Less($close / Ref($close, 1) - 1, 0)), 20) + "
                "Mean(Greater($close / Ref($close, 1) - 1, 0) * "
                "Greater($close / Ref($close, 1) - 1, 0), 20) + 1e-12)) * "
                "(Mean($turnover_rate, 20) / (Mean($turnover_rate, 120) + 1e-12))",
                "downasym_turncrowd20_120",
            ),
            (
