...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] trend_alignment trendflip_fit55",
        feature_expressions=[
            ...
            (
                "((Slope($close, 55) / $close) * Rsquare($close, 55) * "
                "(1 - Mean((($close > Ref($close, 1)) * (Ref($close, 1) < Ref($close, 2))) + "
                "(($close < Ref($close, 1)) * (Ref($close, 1) > Ref($close, 2))), 20))) / "
                "(Std($close / Ref($close, 1) - 1, 55) + 1e-12)",
                "trendflip_fit55",
            ),
            (
