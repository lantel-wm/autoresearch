...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] overnight_gap overnight_risk_tugcalm20_120",
        feature_expressions=[
            ...
            (
                "(Mean($open / Ref($close, 1) - 1, 5) - Mean($close / $open - 1, 5)) * "
                "(Std($open / Ref($close, 1) - 1, 20) / "
                "(Std($close / $open - 1, 20) + 1e-12)) * "
                "(Mean($turnover_rate, 120) / (Mean($turnover_rate, 20) + 1e-12))",
                "overnight_risk_tugcalm20_120",
            ),
            (
