...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][docs] open_to_open_plus_mid_regimes",
        feature_expressions=[
            ...
            ("$close / Ref($close, 10) - 1", "close_return_10"),
            ("$close / Ref($close, 20) - 1", "close_return_20"),
            ("Mean($close, 5) / $close - 1", "ma_gap_5"),
            ("Mean($close, 20) / $close - 1", "ma_gap_20"),
            ("Mean($close, 10) / Mean($close, 20) - 1", "ma_ratio_10_20"),
            ...
            ("Mean($volume, 20) / Mean($volume, 60) - 1", "volume_ratio_20_60"),
            ("Std($volume, 20) / Mean($volume, 20)", "volume_vol_20"),
            ("$turnover_rate", "turnover_rate"),
            ("Mean($turnover_rate, 5)", "turnover_rate_mean_5"),
            ("Mean($turnover_rate, 5) / Mean($turnover_rate, 20) - 1", "turnover_ratio_5_20"),
