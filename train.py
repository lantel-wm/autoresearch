...
def build_experiment() -> ExperimentSpec:
    return ExperimentSpec(
        description="[factor][paper] behavioral_trading losspath_losscrowd55_120",
        feature_expressions=[
            ...
            (
                "Greater(0 - ($close / Ref($close, 55) - 1), 0) * "
                "Greater(Mean($close < Ref($close, 1), 55) - Mean($close > Ref($close, 1), 55), 0) * "
                "(Mean($turnover_rate, 20) / (Mean($turnover_rate, 120) + 1e-12))",
                "losspath_losscrowd55_120",
            ),
            (
